...
cpdef Mesh perform_mesh_csg(Mesh mesh_1, Mesh mesh_2, CSG_Operator operator):
    ...
    m1_intersections = {}
    m1m2_distance = np.empty(n_m1_triangles)
    m1m2_distance[:] = 1E999
    ...
    m2_pair = np.zeros(n_m1_triangles)  # TODO - still needed?
    ...
    for m1_tri_id in range(n_m1_triangles):
        ...
        for m2_tri_id in range(n_m2_triangles):
            ...
            if result[0]:
                ...
                if uv_distance < m1m2_distance[m1_tri_id]:
                    m1m2_distance[m1_tri_id] = uv_distance
                    m1_from_m2_signed_distance[m1_tri_id] = d_u_from_v

                if uv_distance < m2m1_distance[m2_tri_id]:
                    m2m1_distance[m2_tri_id] = uv_distance
    ...
    # TODO - is this section being repeated? Maybe move to end of code
