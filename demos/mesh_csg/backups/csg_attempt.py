...
for s2_tri_id in s2_intersections:
    ...
    # transform to x-y plane
    s2_transform = translate(v1.x, v1.y, v1.z) * rotate_basis(v1v2, n_pi2)
    s2_inv_transform = s2_transform.inverse()

    points = set()
    for vertex in (v1, v2, v3):
        tv = vertex.transform(s2_inv_transform)
        points.add((tv.x, tv.z))

    debug_intersection_points = []
    for intersection in s2_intersections[s2_tri_id]:

        s1_tri_id, ut1, ut2 = intersection

        ut1t = ut1.transform(s2_inv_transform)
        points.add((ut1t.x, ut1t.z))
        ut2t = ut2.transform(s2_inv_transform)
        points.add((ut2t.x, ut2t.z))
    ...
    for vertex in points:
        vertex = Point3D(vertex[0], 0, vertex[1])
        tv = vertex.transform(s2_transform)
        combined_vertices.append([tv.x, tv.y, tv.z])
